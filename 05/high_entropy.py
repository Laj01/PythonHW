...
def is_valid(string):
    ...
    valid = 0
    passphrase = PASSPHRASES.splitlines()
    for lines in passphrase:
        list_of_words = lines.split()
        set_of_words = set(list_of_words)
        if len(set_of_words) == len(list_of_words):
            valid += 1    
...
def main():    
    ...
    print(f"Total number of passphrases: {len(PASSPHRASES.splitlines())}")
    print(f"Total number of VALID passphrases: {is_valid(PASSPHRASES)}")
